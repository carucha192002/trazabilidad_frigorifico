...
if($_POST['funcion']=='capturar_datos'){
    ...
    foreach($usuario->objetos as $objeto) {
        $json[]=array(
            ...
            'sexo'=>$objeto->sexo_us,
            'adicional'=>$objeto->adicional_us,
            'edadfecha'=>$objeto->edad,
            'secretaria'=>$objeto->nombre_tipo
        );
    }
    $jsonstring =json_encode($json[0]);
    echo $jsonstring;
...
if($_POST['funcion']=='editar_usuario'){
    ...
    $usuario->editar($id_usuario,$telefono,$domicilio,$email,$sexo,$adicional,$secretaria,$fnacimiento);
    echo 'editado';
}
...
if($_POST['funcion']=='crear_usuario'){
    ...
    $avatar='default.png';
    $usuario->crear($nombre,$apellido,$edad,$dni,$pass,$rol,$rolnombre,$avatar);
}
if($_POST['funcion']=='ascender'){
    ...
    $rol = $_POST['rol'];
    $rolnombre = $_POST['rolnombre'];
    $usuario->ascender($pass,$id_ascendido,$id_usuario,$rol,$rolnombre);
...
if($_POST['funcion']=='descender'){
    ...
    $rol = $_POST['rol'];
    $rolnombre = $_POST['rolnombre'];
    $usuario->descender($pass,$id_descendido,$id_usuario,$rol,$rolnombre);  
}
...
if($_POST['funcion']=='rellenar_rol'){
    $usuario->rellenar_rol();
    $json=array();
    foreach ($usuario->objetos as $objeto) {
        $json[]=array(
            'id'=>$objeto->id_tipo_us,
...
if($_POST['funcion']=='rellenar_rol1'){
    $usuario->rellenar_rol1();
    $json=array();
    foreach ($usuario->objetos as $objeto) {
        $json[]=array(
            'id'=>$objeto->id_tipo_us,
...
if($_POST['funcion']=='rellenar_rol_modal'){
    $id=$_POST['id'];
    $usuario->rellenar_rol_modal($id);
    $json=array();
    foreach ($usuario->objetos as $objeto) {
        $json[]=array(
            'id'=>$objeto->id_tipo_us,
...
if($_POST['funcion']=='email'){
    $usuario->ver_mail($id_usuario);
    $email_verificacion=$usuario->objetos[0]->email_us;
 if($email_verificacion==''){
     echo '1';
 }else{
     echo '0';
 }
...
if($_POST['funcion']=='cambiar_email'){
    $email=$_POST['login'];
    $usuario->cambiar_email($email,$id_usuario);  
}
?>