...
class Lote{
    ...
    function buscar(){
         ...
         where faenados.salio='NO' 
ORDER BY DATE_ADD(faenados.fecha, INTERVAL conservacion.vida DAY) ASC limit 15";
    ...
    function buscardespachobarras($valor){
        ...
        where id_faenados=:consulta and faenados.salio='NO' and ingresos.ano=faenados.ano";
        $query = $this->acceso->prepare($sql);
    ...
    function buscar_id($id){
       ...
       join despieces on faenados.despiece= despieces.codigo
       join matarife on ingresos.matarife=id_matarife
       where faenados.salio='NO' and id_faenados=:id ORDER BY faenados.garron asc limit 25";
        $query = $this->acceso->prepare($sql);
        $query->execute(array(':id'=>$id)); 
        $this->objetos=$query->fetchall();
        return $this->objetos;      
    ...
    function buscardespachoexiste($valor){
        ...
        join despieces on faenados.despiece= despieces.codigo
        where id_faenados=:consulta and faenados.salio='NO'";
        $query = $this->acceso->prepare($sql);
        $query->execute(array(':consulta'=>$valor)); 
        $this->objetos=$query->fetchall();
